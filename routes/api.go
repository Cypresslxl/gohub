...
func RegisterAPIRoutes(r *gin.Engine) {
	...
	v1.Use(middlewares.LimitIP("200-H")) //每小时最多200个请求
	{
		...
		authGroup.Use(middlewares.LimitIP("1000-H"))
		{
			...
			// 2.verify-code需要限流，邮箱和手机短信限流更严苛，图片验证码限流相对宽松
		...
		authGroup.GET("/user", middlewares.AuthJWT(), user.CurrentUser) //注意这里是在‘auth' 路由下的哦
		usersGroup := v1.Group("/users")
		{
			usersGroup.GET("", user.Index)
			usersGroup.PUT("", middlewares.AuthJWT(), user.UpdateProfile)
			usersGroup.PUT("/email", middlewares.AuthJWT(), user.UpdateEmail)
			usersGroup.PUT("/phone", middlewares.AuthJWT(), user.UpdatePhone)
			usersGroup.PUT("/password", middlewares.AuthJWT(), user.UpdatePassword)
			usersGroup.PUT("/avatar", middlewares.AuthJWT(), user.UpdateAvatar)
		}
		...
		{
			topicsGroup.GET("", topics.Index)
			topicsGroup.GET("/:id", topics.Show)
			topicsGroup.POST("", middlewares.AuthJWT(), topics.Store)
			topicsGroup.PUT("/:id", middlewares.AuthJWT(), topics.Update)
			topicsGroup.DELETE("/:id", middlewares.AuthJWT(), topics.Delete)
		}

		//--4--links
		links := new(controllers.LinksController)
		linksGroup := v1.Group("/links")
		{
			linksGroup.GET("", links.Index)
		}
	}
