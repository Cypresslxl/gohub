// Package cache 缓存工具类，可以缓存各种数据类型包括struct 对象
package cache
...
func Set(key string, obj interface{}, expireTime time.Duration) {
	...
	logger.LogIf(err)
	Cache.Store.Set(key, string(res), expireTime)
}

func Get(key string) interface{} {
	stringValue := Cache.Store.Get(key)
	var res interface{}
	err := json.Unmarshal([]byte(stringValue), &res)
	logger.LogIf(err)
	return res
}
...
func GetObject(key string, res interface{}) {
