...
// UserUpdateAvatarRequest *multipart.FileHeader 是 Govalidator 验证文件必须使用的类型。 form:"avatar" 是指示 gin 在调用 c.ShouldBind() 时，从表单请求中读取。 另外验证规则里，file:avatar ，是 Govalidator 规定了，验证文件必须在字段前方加 file: 前缀。
